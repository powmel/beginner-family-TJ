...
class MoodViewerApp:
    ...
    def color_calendar(self):
        for date in self.records:
            for entry in self.records[date]:
                if entry.startswith("気分"):
                    mood_value = int(entry.split()[1][0])
                    if mood_value > 3:
                        self.calendar.calevent_create(date, '', tags='good_mood')
                    else:
                        self.calendar.calevent_create(date, '', tags='bad_mood')
                        self.calendar.tag_config('good_mood', background='lightgreen')
                        self.calendar.tag_config('bad_mood', background='lightcoral')
